...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // hFov and vFov are degree (transfer to radians)
  double x_camera = (x - 0.5) * 2 * radians((hFov) / 2);
  double y_camera = (y - 0.5) * 2 * radians((vFov) / 2);

  Vector3D cameraPoint = Vector3D(x_camera, y_camera, -1);

  // compute camera space direction vector
  // Vector3D cameraDirection = cameraPoint - Vector3D(0, 0, 0);
  Vector3D cameraDirection = cameraPoint - Vector3D(0, 0, 0);
  Vector3D cameraDirection_n = cameraDirection.unit();

  // compute direction vector of the world ray
  Vector3D worldDirection_n = c2w * cameraDirection_n;
  worldDirection_n.normalize();

  // ray representation: r(t) = o + td
  Ray ray = Ray(position(),  worldDirection_n);
